 ...
 */
export default function ViewOnlyGate({ children, readOnlyRoles = ['student'], label = 'View-only mode' }) {
  const { role } = useAuth();
  ...
  return (
    <div>
      ...
      >
        <Eye size={14} />
        <span className="text-xs font-medium">{label} — students can browse the seat plan but cannot edit.</span>
      </div>
      <fieldset
        disabled
